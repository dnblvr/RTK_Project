...
// TODO: set up fixed point processing
  ...
  class ExtendedComplementaryFilter {
    ...
    public: 
      ...
      inline ExtendedComplementaryFilter(float K_norm, float t_norm, float K_init, float dt) :
        K_norm_(K_norm), t_norm_(t_norm), K_init_(K_init), dt_(dt), t_ratio_(t_norm/dt),
        ref_accel_(0, 0, -1),
        ref_mag_  (0, -1, 0)
      ...
      // pointer version
      void update(States *current_state, States *previous_state) {

        Quaternion q_p      = previous_state->q;

        float accel_norm_n_ = current_state->accel.getMagnitude(),
              mag_norm_n_   = current_state->mag.getMagnitude();

        VectorFloat unit_accel_n  = current_state->accel.getNormalized();
        VectorFloat unit_mag_n    = current_state->mag.getNormalized();
        ...
        K_n_      = find_gain_(current_state->sample);

        gyro_ddt_ = current_state->gyro - total_error_n_*K_n_;
        av_n_     = Quaternion(0, gyro_ddt_.x, gyro_ddt_.y, gyro_ddt_.z);
        q_ddt_    = q_p.getProduct(av_n_) * 0.5f;

        current_state->q = q_p + q_ddt_*dt_;
        current_state->q.normalize();
    ...
    private:
      ...
      inline float find_gain_(float time) {
        return K_norm_   +   (time < t_norm_) * (t_norm_ - time)*(K_init_ - K_norm_)/t_norm_;
      }
      
      inline float find_gain_(unsigned int sample) {
        return K_norm_   +   ((float)sample < t_ratio_) * (t_ratio_ - (float)sample)*(K_init_ - K_norm_)/t_ratio_;
      }
      
      inline float find_gain_(unsigned long long sample) {
        return K_norm_   +   ((float)sample < t_ratio_) * (t_ratio_ - (float)sample)*(K_init_ - K_norm_)/t_ratio_;
      }
